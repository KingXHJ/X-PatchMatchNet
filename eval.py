...
if __name__ == "__main__":
    # Command: python eval.py --input_folder D:\Recent_things\CVlifeMVS-modified\DeepLearningMVS\PatchmatchNet-main\data\hitstar --output_folder D:\Recent_things\CVlifeMVS-modified\DeepLearningMVS\PatchmatchNet-main\output_hitstar --checkpoint_path D:\Recent_things\CVlifeMVS-modified\DeepLearningMVS\PatchmatchNet-main\checkpoints\params_000007.ckpt
    torch.backends.cuda.matmul.allow_tf32 = True
    torch.backends.cudnn.benchmark = True
    torch.backends.cudnn.deterministic = False
    torch.backends.cudnn.allow_tf32 = True
    data = torch.randn([1, 3, 1200, 1600], dtype=torch.float, device='cuda', requires_grad=True)
    net = torch.nn.Conv2d(3, 8, kernel_size=[3, 3], padding=[1, 1], stride=[1, 1], dilation=[1, 1], groups=1)
    net = net.cuda().float()
    out = net(data)
